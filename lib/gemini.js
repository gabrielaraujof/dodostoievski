...
function toSdkHistory(history) {
  // O SDK espera: [{ role: "user"|"model", parts: [{ text }] }]
  // Filtra o primeiro item se for do assistente (startChat não aceita histórico iniciando com "model")
...
export async function generateResponse(userMessage, state) {
  ...
  // Histórico das mensagens anteriores (exceto a atual)
