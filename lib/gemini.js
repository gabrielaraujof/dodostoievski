...
const UNIFIED_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    isSolved: { type: SchemaType.STRING },
...
export async function processUserIntent(userMessage, state) {
  ...
  const generationConfig = {
    ...
    thinkingLevel: "Minimal", // Garante baixa latência para o Telegram
  };
  ...
  try {
    ...
    return {
      ...parsed,
      isSolved: parsed.isSolved === "true" || parsed.isSolved === true
    };
  } catch (e) {
    console.error("Unified Process Error (Gemini 3):", e.message);
...
// ─── Geração de resposta simples (para SINAIS internos) ──────────────────
export async function generateResponse(userMessage, state) {
  const phase = getPhase(state.phase);
  const substate = state.substate || "puzzle";
  const systemPrompt = buildSystemPrompt(phase, substate, state.summary, false);
  ...
  try {
    ...
    console.error("Generate Response Error (Gemini 3):", e.message);
    return "[SINAL: Erro de processamento. Dostoiévski se cala.]";
  }
}
