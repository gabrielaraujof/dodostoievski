...
SUBESTADO: ${substate === "puzzle" ? "Resolvendo Puzzle no Mini App" : "Conversando/Debriefing"}
...
export async function shouldAdvancePhase(userMessage, aiResponse, state) {
  ...
  // Só avançamos para a próxima fase se estivermos no subestado de chat
  ...
  const model = genAI.getGenerativeModel({
    model: "gemini-2.0-flash",
    systemInstruction: `
      Você é um avaliador de conversas. Responda APENAS com "sim" ou "não".
      Avalie se o diálogo deve ser encerrado para que o usuário avance para o próximo desafio.
      Condição de avanço: ${phase.advanceHint}
      
      Mensagem da usuária: "${userMessage}"
      Resposta do Dostoiévski: "${aiResponse}"
      
      A conversa chegou ao ponto de avanço?
    `,
  });
