...
REJEITE: "não sei", "?", "observe", apenas "the cat" sem identificar como felino, ou divagações técnicas sobre a cifra sem nomear o subjeito.`,
    ...
    // entre eles pelo servidor (bolha do meio). O próprio token da cifra e a
    // sua decodificação são OMITIDOS do texto dos sinais — mantê-los fora do
    // contexto do LLM é a forma mais robusta de evitar que o LLM os reproduza
    // inline em alguma bolha. Referências são indiretas ("a sequência de
    // letras que vai aparecer", "the message above").
...
Em UMA ÚNICA bolha curta (no máximo 25 palavras), em primeira pessoa, EXCLUSIVAMENTE em inglês, no seu tom ácido e cerebral habitual, avise a usuária que uma sequência de letras vai aparecer logo após a sua bolha, e que ela precisa decodificá-la rotacionando cada letra por exatamente metade do alfabeto latino. NUNCA nomeie a cifra (não escreva "ROT-13", "ROT13", "Caesar", "César") — descreva apenas o mecanismo.
...
NUNCA afirme que ela acertou nada (ela ainda nem respondeu esta fase). NUNCA mencione fones, conserto, silenciado, repair ou silent thing.]`,
    puzzleSignalPost: `[SINAL DE ENIGMA PÓS-CIFRA: Uma sequência curta de letras maiúsculas acabou de ser enviada numa bolha isolada logo acima da sua. Em UMA ÚNICA bolha curta (no máximo 25 palavras), em primeira pessoa, EXCLUSIVAMENTE em inglês, no seu tom ácido e cerebral, instrua a usuária a obedecer à instrução que aparecerá depois de decodificar — nomear o subjeito em qualquer idioma, cirílico ainda preferido.

Refira-se à sequência apenas como "the message above", "that string" ou "the letters". NUNCA entregue a resposta nem qualquer pista direta (não escreva cat, chat, gato, koshka, kot, quissa, observe).
